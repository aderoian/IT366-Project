...
void input_update(struct widget_s *element, float deltaTime);
void input_destroy(struct widget_s *element);
void input_draw(struct widget_s *element);

widget_t * input_create(const char *id, GFC_Vector2D position, GFC_Vector2D size, const char *placeholder, int fontSize) {
    widget_t *element = widget_create(id);
    if (!element) {
        log_error("Failed to allocate memory for input field element");
        return NULL;
    }
    ...
    element->type = TYPE_INPUT;
    element->rect = gfc_rect(position.x, position.y, size.x, size.y);
    element->data = data;
    element->visible = 1;

    element->update = input_update;
    element->draw = input_draw;
    element->destroy = input_destroy;

    return element;
}

const char * input_get_text(struct widget_s *element) {
    if (!element) {
        return NULL;
    }

    input_field_data_t *data = (input_field_data_t *)element->data;
    if (data->text && strlen(data->text->buffer) > 0) {
        return data->text->buffer;
    }
...
void input_update(struct widget_s *element, float deltaTime) {
    ...
    if (data->focused) {
        ...
        char test[2] = "\0\0";
        if (mouseState & SDL_BUTTON(SDL_BUTTON_LEFT)) {
            if (!gfc_point_in_rect(gfc_vector2d(x, y), element->rect)) {
                data->focused = 0; // Set focus to this input field
                return;
            }
        }
...
void input_draw(struct widget_s *element) {
    ...
    if (data->focused) {
        // Draw the input field background (you can use a sprite or a simple rectangle)
        gf2d_draw_rect_filled(element->rect, gfc_color(0.8, 0.8, 0.8, 1)); // Light gray background

        // Draw the text
        if (data->text && strlen(data->text->buffer) > 0) {
            gf2d_font_draw_text(data->fontSize, data->text->buffer, gfc_vector2d(element->rect.x + 5, element->rect.y + 5));
        } else {
            // Draw placeholder text if no input
            gf2d_font_draw_text(data->fontSize, data->placeholder->buffer, gfc_vector2d(element->rect.x + 5, element->rect.y + 5));
        }
    } else {
        // Draw the input field background (you can use a sprite or a simple rectangle)
        gf2d_draw_rect_filled(element->rect, gfc_color(0.9, 0.9, 0.9, 1)); // Lighter gray background
